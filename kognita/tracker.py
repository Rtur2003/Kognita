...
stop_flag = Event()
POLL_INTERVAL = 3
IDLE_THRESHOLD_SECONDS = 180 # Default value, will be updated from config
last_activity_time = time.time()

def update_settings(config):
    """Updates tracker settings from the config file."""
    global IDLE_THRESHOLD_SECONDS
    IDLE_THRESHOLD_SECONDS = config.get('settings', {}).get('idle_threshold_seconds', 180)
    print(f"Idle threshold set to: {IDLE_THRESHOLD_SECONDS} seconds")
...
def log_activity(conn, process_name, title, start_time, end_time):
    ...
    if duration < POLL_INTERVAL or process_name in ['idle', 'unknown']: return
    cursor = conn.cursor()
    cursor.execute(
...
def start_tracking():
    ...
    try:
        while not stop_flag.is_set():
            ...
            if current_process_name != last_process_name or current_window_title != last_window_title:
                ...
                with get_db_connection() as conn:
                    if conn: log_activity(conn, last_process_name, last_window_title, session_start_time, session_end_time)
                session_start_time = time.time()
        ...
        if last_process_name is not None:
            ...
            with get_db_connection() as conn:
                if conn: log_activity(conn, last_process_name, last_window_title, session_start_time, final_end_time)
